...
def test_heap_sort():
    ...
    A = shuffled_list(1000, 0)
    heap.heapSort(A)
    print("Correctly sorted 1000 shuffled: {}".format(is_sorted(A)))


def within_20_percent(expected, actual):
    margin = 0.2 * expected
    low = expected - margin
    hi = expected + margin
...
if __name__ == '__main__':
    ...
    parser.add_argument("--test", dest="test_number",
                        type=int, help="The number of the test to run [1-5]")
